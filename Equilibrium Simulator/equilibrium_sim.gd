...
func _process(delta):
	...
	# literally rate laws
		...
		if(6.5< H2CO3_concentration and H2CO3_concentration < 7.0):
			done.emit()
...
func _on_increase_1_pressed():
	if(not paused):
		H2O_concentration = min(10.0,H2O_concentration + 0.1)
...
func _on_increase_2_pressed():
	if(not paused):
		CO2_concentration = min(10.0,CO2_concentration + 0.1)
